...
#define Ourselves_M35_Auto1_cpp
#ifdef Ourselves_M35_Auto1_cpp
#include "raspberry.h"
#include "string"
#include "vector"
#include "TOF.hpp"
#include "ACFLY_cmd.hpp"
using namespace std;
LMZ_TOF myTofSence;
#endif
...
#ifdef Ourselves_M35_Auto1_cpp
	rpi_setBauds(57600);  // 树莓派串口波特率初始化
	LMZ_TOF::TOF_setBauds(921600); // TOFSence 串口波特率初始化
	ACFLYcmd rpiCmd;
	bool sendMessageToRpi = true ;
#endif
...
//#define TOFSenseTest
				...
				if(sendMessageToRpi){
					...
					LMZ_SendMessage("EnterMode");
					LMZ_SendMessage("EnterMode");
					rpiCmd.Curcmd.clearAllData();
					rpiCmd.freeStatus = true ;
					sendMessageToRpi = false ;
				}
				
//				double dis = myTofSence.getTOFdistance();
//				LMZ_SendMessage("acfly: distance = " + to_string(dis) + '\n');

				// 接收命令
				string str_commnd ;
				if(rpiCmd.freeStatus){
					// 接收来自树莓派的命令
					str_commnd = receiveData();
				}
